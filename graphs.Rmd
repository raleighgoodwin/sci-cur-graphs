...
j_tempplot_df <- tempplot_df %>% 
  mutate(No_Smoothing = No_Smoothing+17,
         title = "Average Area of Sand on Earth\n1980-2021")
...
j_tempplot <- j_tempplot + 
  labs(x = NULL,
       y = "Sandy Land Area (million square miles)") +
  facet_grid(. ~ title) +
  theme_linedraw() +
  theme(
    axis.text.x = element_text(face = "bold",
    ...
    panel.grid.major = element_line(color = "black", size = .3)) +
  scale_x_continuous(breaks = seq(1980, 2021, by = 2)) +
  scale_y_continuous(limits = c(17, 18.5), breaks = seq(17, 18.5, by = .1))
